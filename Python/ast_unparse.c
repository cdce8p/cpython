...
static int
append_ast_cascade_subscript(PyUnicodeWriter *writer, expr_ty e)
{
    APPEND_CHAR('[');
    APPEND_EXPR(e->v.CascadeSubscript.slice, PR_TUPLE);
    APPEND_CHAR_FINISH(']');
}
...
append_ast_expr_dispatch(PyUnicodeWriter *writer, expr_ty e, int level)
{
    ...
    case CascadeSubscript_kind:
        return append_ast_cascade_subscript(writer, e);
