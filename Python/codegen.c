...
static int codegen_none_aware_access(compiler *, expr_ty);
...
codegen_visit_expr(compiler *c, expr_ty e)
{
    ...
    case NoneAwareAccess_kind:
        return codegen_none_aware_access(c, e);
    case Subscript_kind:
...
static int
codegen_none_aware_access(compiler *c, expr_ty e)
{
    assert(e->kind == NoneAwareAccess_kind);
    NEW_JUMP_TARGET_LABEL(c, end);
    location loc = LOC(e);
    _PyCompile_PushNoneAwareJumpTarget(c, end);
    jump_target_label next = _PyCompile_TopNoneAwareJumpTarget(c);

    VISIT(c, expr, e->v.NoneAwareAccess.value);
    _PyCompile_PopNoneAwareJumpTarget(c);
    ADDOP_I(c, loc, COPY, 1);
