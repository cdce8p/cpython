    ...
    for (i = begin; i < end; i++) {
        ...
        if (!map_built
            && (key->kind == NoneAwareElement_kind
                || key->kind == IfElement_kind
                || value->kind == NoneAwareElement_kind
                || value->kind == IfElement_kind)) {
        ...
        } else if (key->kind == IfElement_kind) {
            RETURN_IF_ERROR(
                codegen_jump_if(c, loc, key->v.IfElement.test, end, 0));
            VISIT(c, expr, key->v.IfElement.item);
        } else {
            VISIT(c, expr, key);
        }
        if (value->kind == NoneAwareElement_kind) {
            ...
            RETURN_IF_ERROR(
                codegen_jump_if(c, loc, value->v.IfElement.test, cleanup1, 0));
            VISIT(c, expr, value->v.IfElement.item);
        } else {
            VISIT(c, expr, value);
        }
...
codegen_formatted_value(compiler *c, expr_ty e)
{
    ...
    if (value->kind == NoneAwareElement_kind || value->kind == IfElement_kind) {
        ADDOP_JUMP(c, NO_LOCATION, JUMP_NO_INTERRUPT, end);
